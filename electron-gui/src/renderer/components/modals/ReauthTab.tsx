...
export function ReauthTab() {
  ...
  // Load config on mount to get default snapshot name.
  useEffect(() => {
  ...
  return (
    <div className={step === 'ssh' ? 'flex flex-col h-full' : 'flex items-start justify-center h-full overflow-y-auto py-8'}>
      <div className={step === 'ssh' ? 'flex flex-col h-full w-full px-4 py-4 space-y-4' : 'w-full max-w-md space-y-4 px-4'}>
        <div>
          <h2 className="text-lg font-semibold">Re-Authenticate</h2>
        ...
        {step === 'ssh' && preparedDevbox && sshSpawnInfo && (
          <div className="flex-1 flex flex-col space-y-4 min-h-0">
            ...
            <div className="flex-1 flex rounded-md overflow-hidden border border-border/50 min-h-0">
              <LiveTerminalView
