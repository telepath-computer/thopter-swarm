...
if [ -f /tmp/thopter-active ] && [ "$(find /tmp/thopter-active -mmin -1 2>/dev/null)" ]; then
    /usr/local/bin/thopter-status-line running >/dev/null 2>&1 || true
else
    CURRENT_STATUS=$(redis-cli --tls -u "$THOPTER_REDIS_URL" GET "thopter:${THOPTER_NAME}:status" 2>/dev/null || true)
    if [ "$CURRENT_STATUS" = "running" ]; then
        /usr/local/bin/thopter-status-line inactive >/dev/null 2>&1 || true
    fi
fi

for i in 1 2 3 4 5 6; do
    /usr/local/bin/thopter-status-line heartbeat >/dev/null 2>&1
