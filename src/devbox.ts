...
export async function createDevbox(opts: {
  ...
  try {
    ...
    // Identity vars
    envLines.push(`export THOPTER_NAME="${opts.name}"`);
    envLines.push(`export THOPTER_ID="${devbox.id}"`);
    envLines.push(`export THOPTER_OWNER="${ownerName}"`);
    const ntfyChannel = getNtfyChannel();
    if (ntfyChannel) {
      envLines.push(`export THOPTER_NTFY_CHANNEL="${ntfyChannel}"`);
    }
